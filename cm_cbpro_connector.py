...
def get_order_history(acct_id, acct_canonical_name=None, cb_profile_name='coinbase_sandbox', rate_limit=0.5):
    """
    ...
    auth_client = cbpro.AuthenticatedClient(**cm_secrets.get_coinbase_credentials(cb_profile_name=cb_profile_name))
    i = 0
    if acct_canonical_name:
        acct_name = acct_canonical_name
    else:
        acct_name = acct_id
    ...
    # logger.info(f"Got {len(order_ids)} order ids.")
    return order_ids
...
#     acct_id = '3fa53d16-7da5-4272-bfbf-bc9cc8ac698e'
    rate_limit = 0.21
    logger.info("Start")
    accounts = get_account_info('coinbase_secrets', rate_limit=rate_limit)
    acct_ids = []
    order_id_list = []
    ...
    for acct in accounts:
        ...
        _ids = get_order_history(acct['id'],acct_canonical_name=acct['currency'], cb_profile_name='coinbase_secrets', rate_limit=rate_limit)
        d['order_ids'] = _ids
    ...
    for a in order_id_list:
        for o in a['order_ids']:
            od = get_order_details(o,cb_profile_name='coinbase_secrets')
